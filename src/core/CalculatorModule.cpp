#include "headers/CalculatorModule.h"
...
void CalculatorModule::registerBuiltinOperations() {
...
void CalculatorModule::registerOperation(std::shared_ptr<IOperation> operation) {
    std::string name = operation->getName();
    operations[name] = std::move(operation);
}

void CalculatorModule::loadPlugins() {
...
float CalculatorModule::calculate(std::string input) {
